    ...
    if (match) {
        ...
        return (
            <span className="flex items-center gap-1.5 flex-wrap">
                <span className="inline-block bg-[#ffbc0d] text-black px-1 py-0.5 rounded shadow-sm text-[10px] font-black">{match[0]}</span>
...
export default function KitchenDashboard({ activeOrders, updateStatus, printTicket, brand, settings, profile }) {
    ...
    const getSourceStyles = (source) => {
        ...
        if (source === 'glovo') return {
            border: 'border-[#ffbc0d]',
            bg: 'bg-yellow-900/20',
            headerBg: 'bg-[#ffbc0d]',
            headerBorder: 'border-black/20',
            text: '!text-[15px] text-black',
            label: <img src="https://cdn.worldvectorlogo.com/logos/glovo.svg" alt="GLOVO" className="h-8 sm:h-9 object-contain drop-shadow-md" onError={(e) => { e.target.onerror = null; e.target.src = 'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e0/Glovo_logo.svg/512px-Glovo_logo.svg.png'; }} />,
                            ...
                            <div key={o.id} className={`${styles.bg} border-2 ${styles.border} rounded-2xl flex flex-col overflow-hidden shadow-lg`}>
                                ...
                                <div className="p-3 flex-1 no-scrollbar space-y-1.5">
                                    ...
                                    {o.orderNote && (() => {
                                        ...
                                        return (
                                            <div className="mt-2 text-[11px] font-bold text-black bg-[#ffbc0d] p-2.5 rounded-lg border-2 border-yellow-500 flex flex-col gap-1 shadow-md">
                                                {validLines.map((line, idx) => {
                                                    const lowerLine = line.toLowerCase();
                                                    if (lowerLine.includes('sans ')) {
                                                        return <span key={idx} className="text-red-700 font-black bg-white/50 px-1.5 py-0.5 rounded inline-block">🚫 {line.trim()}</span>;
                                                    } else if (line.match(/^\d+/) || lowerLine.includes('bocadillo') || lowerLine.includes('tacos')) {
                                                        return <span key={idx} className="text-green-800 font-bold mt-1 inline-block">🍔 {line.trim()}</span>;
                                                    } else {
                                                        return <span key={idx} className="text-black pl-1 inline-block">{line.trim()}</span>;
                                                    }
                                                })}
                                            </div>
                                        );
                        ...
                        <div key={o.id} className={`${styles.bg} border-2 ${styles.border} transition-colors rounded-3xl flex flex-col overflow-hidden shadow-2xl`}>
                            ...
                            <div className="p-3 sm:p-4 flex-1 space-y-2 no-scrollbar">
                                            ...
                                            <div className="flex gap-4 items-start">
                                                ...
                                                <div className="flex-1 pt-1">
                                                    <span className={`font-black text-2xl flex items-start gap-2 leading-tight ${isChecked ? 'line-through decoration-2' : ''} ${(baseName.toLowerCase().includes('sans') ? 'text-red-500' : (baseName.toLowerCase().includes('extra') || baseName.toLowerCase().includes('ajout')) ? 'text-green-500' : 'text-white')}`}>
                                                        <span className="inline-block bg-[#ffbc0d] text-black px-2 py-0.5 rounded-lg shadow-sm font-black mr-2 text-xl">{item.qty}x</span>
                                ...
                                {o.orderNote && (() => {
                                    ...
                                    if (validLines.length === 0) return null;
                                    return (
                                        <div className="mt-6 p-5 bg-[#ffbc0d] border-4 border-yellow-500 rounded-2xl shadow-xl">
                                            <span className="text-sm font-black uppercase tracking-widest text-black flex items-center gap-2 mb-2"><AlertTriangle size={20}/> Note Client Spéciale :</span>
                                            <div className="text-lg font-black text-black space-y-2">
                                                {validLines.map((line, idx) => {
                                                    const lowerLine = line.toLowerCase();
                                                    if (lowerLine.includes('sans ')) {
                                                        return <div key={idx} className="text-red-700 font-black text-base flex items-center gap-2">🚫 <span className="bg-white/50 px-2 py-0.5 rounded">{line.trim()}</span></div>;
                                                    } else if (line.match(/^\d+/) || lowerLine.includes('bocadillo') || lowerLine.includes('tacos') || lowerLine.includes('burger')) {
                                                        return <div key={idx} className="text-green-900 font-black text-lg mt-3 flex items-center gap-2">🍔 {line.trim()}</div>;
                                                    } else {
                                                        return <div key={idx} className="text-black ml-6">{line.trim()}</div>;
                                                    }
                                                })}
                                            </div>
                                        </div>
