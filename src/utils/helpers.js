...
export const translateOptionKDS = (text, isExtra, enableArabic) => {
    if (!text) return '';
    ...
    if (enableArabic) {
        if (!isExtra) {
            ...
            if (lower.includes('هريسة')) return '🌶️ بلا هريسة';
            ...
            else if (lower.includes('harissa') || lower.includes('hrissa')) translated = '🌶️ هريسة';
            ...
            else if (lower.includes('charcuterie')) translated = '🥓 الكاشير';
            else if (lower.includes('saucisse')) translated = '🌭 الصوصيص';
    ...
    // French (default)
    if (!isExtra) {
        if (lower.includes('sans')) return text.toUpperCase();
        return `SANS ${text.toUpperCase()}`;
    } else {
        if (!text.trim().startsWith('+')) return `+ ${text.toUpperCase()}`;
