...
function replaceSafely(file, searchStrOrRegex, replaceStr) {
    let content = fs.readFileSync(file, 'utf8');
    if (typeof searchStrOrRegex === 'string') {
        if (!content.includes(searchStrOrRegex)) {
            console.log('Skipped string: not found in ' + file);
            return;
        }
        content = content.replace(searchStrOrRegex, replaceStr);
    } else {
        if (!searchStrOrRegex.test(content)) {
            console.log('Skipped regex: not found in ' + file);
            return;
        }
        content = content.replace(searchStrOrRegex, replaceStr);
    }
    fs.writeFileSync(file, content);
    console.log('Applied replacement to ' + file);
}
...
replaceSafely(file,
    ...
    'const [showGlovoModal, setShowGlovoModal] = useState(false);\n    const [showCreateGlovoModal, setShowCreateGlovoModal] = useState(false);\n    const [glovoForm, setGlovoForm] = useState({ orderNum: \'\', payment: \'espece\' });'
);

// 2. Rewrite handleGlovoOrderFromPOS
replaceSafely(file,
    /const handleGlovoOrderFromPOS = async \(\) => \{[\s\S]*?showNotify\("W9e3 mochkil f tsjal dyal l-commande Glovo", "error"\);\s*\}\s*\};/,
...
// 3. Change "Client Glovo" inside Telephone modal to open the Create Modal
replaceSafely(file,
    /onClick=\{handleGlovoOrderFromPOS\}[\s\n]*className="w-full mt-3 bg-yellow-400/,
    `onClick={() => { if(cart.length===0) return showNotify("Panier vide", "error"); setShowTelNumpad(false); setShowCreateGlovoModal(true); }}
                                className="w-full mt-3 bg-yellow-400`
);

// 4. Update showGlovoModal to add the "Saisir Commande Glovo" button and new UI
replaceSafely(file,
    /\{\/\* MODAL COMMANDES PRÊTES GLOVO \*\/\}[\s\S]*?\{\/\* MODAL COMMANDES PRÊTES CAISSE \*\/\}/,
    `{/* MODAL CREER COMMANDE GLOVO */}
